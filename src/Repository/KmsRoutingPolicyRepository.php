...
namespace BlackCat\Database\Packages\KmsRoutingPolicies\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\KmsRoutingPolicies\Definitions;
use BlackCat\Database\Packages\KmsRoutingPolicies\Criteria;
use BlackCat\Database\Packages\KmsRoutingPolicies\Dto\KmsRoutingPolicyDto as Dto;
use BlackCat\Database\Packages\KmsRoutingPolicies\Mapper\KmsRoutingPolicyDtoMapper as RowMapper;
...
final class KmsRoutingPolicyRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\KmsRoutingPolicies\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'priority', 'strategy', 'match', 'providers', 'active' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'name' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\KmsRoutingPolicies\Definitions::table());
            return $bulk->upsertMany($rows, [ 'name' ], [ 'priority', 'strategy', 'match', 'providers', 'active' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KmsRoutingPolicies\Dto\KmsRoutingPolicyDto|null */
    public function getByName(string $name, bool $asDto = false): array|\BlackCat\Database\Packages\KmsRoutingPolicies\Dto\KmsRoutingPolicyDto|null {
        $row = $this->getByUnique([ 'name' => $name ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\KmsRoutingPolicies\Mapper\KmsRoutingPolicyDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KmsRoutingPolicies\Dto\KmsRoutingPolicyDto|null */
    public function getByConstraint(mixed $constraint, bool $asDto = false): array|\BlackCat\Database\Packages\KmsRoutingPolicies\Dto\KmsRoutingPolicyDto|null {
        $row = $this->getByUnique([ 'constraint' => $constraint ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\KmsRoutingPolicies\Mapper\KmsRoutingPolicyDtoMapper::fromRow($row);
    }
    public function existsByConstraint(mixed $constraint): bool {
        $where = 't.' . Ident::q($this->db, 'constraint') . ' = :uniq_constraint';
        return $this->exists($where, [ 'uniq_constraint' => $constraint ]);
    }
    /** @return int|string|null */
    public function getIdByConstraint(mixed $constraint) {
        $row = $this->getByConstraint($constraint);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KmsRoutingPolicies\Dto\KmsRoutingPolicyDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\KmsRoutingPolicies\Dto\KmsRoutingPolicyDto|null {
