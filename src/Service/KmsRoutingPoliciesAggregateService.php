...
namespace BlackCat\Database\Packages\KmsRoutingPolicies\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\KmsRoutingPolicies\Dto\KmsRoutingPolicyDto;
use BlackCat\Database\Packages\KmsRoutingPolicies\Mapper\KmsRoutingPolicyDtoMapper;
 ...
 */
final class KmsRoutingPoliciesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private KmsRoutingPolicyRepository $kmsRoutingPolicyRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'kms_routing_policies' => $this->kmsRoutingPolicyRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\KmsRoutingPolicies\ModuleException("Record {$idStr} not found for locking.");
            }
