...
use BlackCat\Database\Packages\KmsRoutingPolicies\Repository\KmsRoutingPolicyRepositoryInterface;
...
class KmsRoutingPolicyRepository implements KmsRoutingPolicyRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'name' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\KmsRoutingPolicies\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'priority', 'strategy', 'match', 'providers', 'active' ]);
          return count($rows);
        }
    ...
    public function getByName(string $name, bool $asDto = false): array|\BlackCat\Database\Packages\KmsRoutingPolicies\Dto\KmsRoutingPolicyDto|null {
        return $this->getByUnique([ 'name' => $name ], $asDto);
    }
